...
```{r read in data}
voter_sample <- read_csv("~/Desktop/Reed_Senior_Thesis/Data_and_results/data/voter_sample.csv")

county_demographics <- read_csv("~/Desktop/Reed_Senior_Thesis/Data_and_results/data/colorado_demographic_stats_by_county.csv")

voter_sample <- left_join(voter_sample, county_demographics, by = "COUNTY")

voter_sample$VOTER_ID <- as.factor(voter_sample$VOTER_ID)
voter_sample$COUNTY <- as.factor(voter_sample$COUNTY)
voter_sample$PARTY <- as.factor(voter_sample$PARTY)
voter_sample$GENDER <- (voter_sample$GENDER == "Male")
voter_sample$ELECTION_TYPE <- as.factor(voter_sample$ELECTION_TYPE)
voter_sample$ELECTION_DATE <- as.factor(voter_sample$ELECTION_DATE)

voter_sample <- mutate(voter_sample,
                       ELECTION = paste(voter_sample$ELECTION_TYPE, voter_sample$ELECTION_DATE, sep = ""))

voter_sample$ELECTION <- as.factor(voter_sample$ELECTION)
```
...
```{r county models}
#Model 1
m1_c <- glm(data = voter_sample, family = "binomial", voted ~ COUNTY)
summary(m1_c)

#Model 2
m2_c <- glmer(data = voter_sample, family = binomial, 
              voted ~ (1|COUNTY))
display(m2_c)  

#Model 3
m3_c <- glm(data = voter_sample, family = "binomial", 
            voted ~ COUNTY + PCT_WHITE + PCT_URBAN)
summary(m3_c)

#Model 4
m4_c <- glmer(data = voter_sample, family = binomial, 
...
```{r}
#Model 1
#m1_i <- glm(data = voter_sample, family = "binomial", voted ~ VOTER_ID)
#Does not work

#Model 2
m2_i <- glmer(data = voter_sample, family = "binomial", 
            voted ~ (1|VOTER_ID))
display(m2_i)

#Model 4
m4_i <- glmer(data = voter_sample, family = "binomial", 
            voted ~ (1|VOTER_ID) + GENDER)
display(m4_i)

#It's hard to estimate models because
length(unique(voter_sample$VOTER_ID))
```
...
```{r}
#Model 3
m3_l <- glm(data = voter_sample, family = "binomial", 
            voted ~ ELECTION_TYPE + ELECTION_DATE)
summary(m3_l)

#Model 4
m4_l <- glmer(data = voter_sample, family = "binomial", 
            voted ~ (1|ELECTION) + ELECTION_DATE + ELECTION_TYPE)
display(m4_l)
```
...
m_ballot_level <- glm(data = voter_sample, family = "binomial", 
                      voted ~ GENDER + PARTY + AGE)
summary(m_ballot_level)
```
...
#Model 2
#Issues with model matrix!
m2_full <- glmer(data = voter_sample, family = "binomial", 
                 voted ~ (1|COUNTY) + (1|ELECTION) + GENDER + AGE +
                   PARTY + PCT_WHITE + PCT_URBAN + MAIL_VOTE)
display(m2)
```
