...
I did this. Still kindof a disaster.
   
   
####By County Cluster-disaster graph
```{r, fig.align= 'center'}
...
ggplot(turnouts_by_county, aes(x = ELECTION_YEAR, y = TURNOUT, col = COUNTY)) +
  ...
  geom_point(size = .2) +
  geom_line(alpha = .3) +
  labs(title = "Turnout in Colorado Elections by County, 1992-2017", x = "Election Year", y = "Turnout as % of Registared Voters") +
  guides(col=FALSE)


##A graph with only the largest 7 counties colorized
big_seven_turnouts <- turnouts_by_county %>%
  filter(COUNTY %in% c("Jefferson", "El Paso", "Denver", "Arapahoe", "Adams", "Larimer", "Boulder"))
...
ggplot(big_seven_turnouts, aes(x = ELECTION_YEAR, y = TURNOUT, col = COUNTY)) +
  ...
  geom_point(size = .2) +
  geom_line(alpha = .8) +
  labs(title = "Turnout in Colorado Elections for the Largest 7 Counties, 1992-2017", x = "Election Year", y = "Turnout as % of Registared Voters")
...
#    mutate(BIG_SEVEN <- ifelse(COUNTY %in% c("Jefferson", "El Paso", "Denver", "Arapahoe", "Adams", "Larimer", "Boulder"), "Big Seven", "Other"))
#  
#  names(grayscale_turnouts)[8] <- "BIG_SEVEN"
#  
#  ggplot(grayscale_turnouts, aes(x = ELECTION_YEAR, y = TURNOUT, col = BIG_SEVEN)) +
