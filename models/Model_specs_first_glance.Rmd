...
$\bullet ~y^{election~type}$, for each election type (Midterm, Primary , Coordinated, General).  
    
$\bullet ~y^{election~year}$, for each election year, between 2010 and 2016.
   
I will also use $year$ as a variable for models using smoothing splines. All election level predictors will be summarized for the purposes of modelling in the 9 by 2 matrix $Y = (y^{election~type}, y^{election~year})$.
...
$$y^{turnout~\%}_{c, l} \sim x^{mail~vote~\%}_{c, l}\beta_1 + \sum_{k = 1}^{4}y_{k, l}^{election~type}\beta_{k+1} + \sum_{j = 1}^{7}y_{j, l}^{election~year}\beta_{j+5} + \sum_{c = 1}^{64} x_c\beta_{c+13} $$ 
...
$$y^{turnout~\%}_{c, l} \sim x^{mail~vote~\%}_{c, el}\beta_1 + \sum_{k = 1}^{4}y_{k, l}^{election~type}\beta_{k+1} + \sum_{j = 1}^{7}y_{j, l}^{election~year}\beta_{j+5} + \sum_{c = 1}^{64} x_c\beta_{c+13} + x^{white~\%}_{c}\beta_{78} + x^{urban~\%}_{c}\beta_{79}$$ 
...
$$y^{turnout~\%}_{c, l} \sim x^{mail~vote~\%}_{c, l}\beta_1 + \sum_{k = 1}^{4}y_{k, l}^{election~type}\beta_{k+1} + \sum_{j = 1}^{7}y_{j, l}^{election~year}\beta_{j+5} + \alpha_{[c]}$$ 
...
$$y^{turnout~\%}_{c, l} \sim x^{mail~vote~\%}_{c, l}\beta_1 + \sum_{k = 1}^{4}y_{k, l}^{election~type}\beta_{k+1} + \sum_{j = 1}^{7}y_{j, l}^{election~year}\beta_{j+5} + \sum_{c = 1}^{n} x^{county~classification}_c\beta_{c+13}$$ 
...
$$x^{mail~vote~\%}_{c, l} \sim \sum_{k = 1}^{4}y_{k, l}^{election~type}\beta_{k} + \sum_{j = 1}^{7}y_{j, l}^{election~year}\beta_{j+4} + \alpha_{[c]}$$ 
$$\alpha_{c} \sim N(x^{white~\%}\gamma_{1} + x^{urban~\%}\gamma_{2}, \sigma_{\alpha}^2),~for~c = 1,...,64$$
...
$$\alpha_{c} \sim N(X_c \gamma, \sigma_{\alpha}^2),~for~c = 1,...,64$$
