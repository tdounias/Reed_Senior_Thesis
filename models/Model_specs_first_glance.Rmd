...
$\bullet~z^{age}$, a vector of age for each voter, varying by voter and election.  
...
Since the unit of observation for the individual level models I will apply are all individuals in a particular election, I define an aggregate matrix of length equal to the product of elections and voters, a.nd width equal to 3. This matrix includes all individual level predictors: $Z =(z^{gender}, z^{age}, z^{party})$. The fourth variable defined in this section is the response variable in the individual level model, and as such is not included in the predictors.
...
$$y^{turnout~\%} \sim y^{mail~vote~\%}\beta_1 + \sum_{i = 1}^{4}y_i^{election~type}\beta_{i+1} + \sum_{j = 1}^{7}y_j^{election~year}\beta_{j+5} + \sum_{c = 1}^{n} x^{county~classification}_c\beta_{c+13}$$ 
