...
Conveniently, despite the use of a link function, there is an easy way to interpret the coefficients of such a regression. While obviously individual values from the $B$ vector will not be particularly helpful, $e^B$ can be used as a vector of multiplicative, one-unit shifts in the value of the probability that $Y = 1$. This means that a one unit increase in any predictor will cause an effect equal to multiplying p by the exponent of the corresponding coefficient^[This can be simplified even more, if one approximates the process of exponentiation with just dividing the coefficient by 4. Crude, yet effective for a quick scan of the results]. [@james_introduction_2017]
