...
% Everything below added by CII
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{md_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ model_dt, turnout }\OperatorTok{~}\StringTok{ }\NormalTok{pct_white }\OperatorTok{+}\StringTok{ }\NormalTok{pct_urban }\OperatorTok{+}\StringTok{ }\NormalTok{county)}
  
  \KeywordTok{summary}\NormalTok{(md_}\DecValTok{1}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#If run this shows perfect linear relationship}
  \CommentTok{#alias(md_1)}
  
  \CommentTok{#plot(md_1)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{md_}\DecValTok{2}\NormalTok{ <-}\StringTok{ }\KeywordTok{lmer}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ model_dt, turnout }\OperatorTok{~}\StringTok{ }\NormalTok{pct_white }\OperatorTok{+}\StringTok{ }\NormalTok{pct_urban }\OperatorTok{+}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{county),}
               \DataTypeTok{REML =}\NormalTok{ F)}
  
  \NormalTok{arm}\OperatorTok{::}\KeywordTok{display}\NormalTok{(md_}\DecValTok{2}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#Run for specific county coefs}
  \CommentTok{#ranef(md_2)}
  
  \KeywordTok{fixef}\NormalTok{(md_}\DecValTok{2}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#plot(md_2)}
  
  \CommentTok{#qqnorm(residuals(md_2))}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{md_}\DecValTok{3}\NormalTok{ <-}\StringTok{ }\KeywordTok{lmer}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ model_dt, turnout }\OperatorTok{~}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{types }\OperatorTok{+}\StringTok{ }\NormalTok{pct_vbm }\OperatorTok{+}
  \StringTok{               }\NormalTok{pct_urban }\OperatorTok{+}\StringTok{ }\NormalTok{pct_white }\OperatorTok{+}\StringTok{ }\NormalTok{pct_vbm}\OperatorTok{:}\NormalTok{types }\OperatorTok{+}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{county), }
               \DataTypeTok{REML =}\NormalTok{ F)}
  
  \NormalTok{arm}\OperatorTok{::}\KeywordTok{display}\NormalTok{(md_}\DecValTok{3}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#Run for county coefs}
  \CommentTok{#ranef(md_3)}
  
  \KeywordTok{fixef}\NormalTok{(md_}\DecValTok{3}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#plot(md_3)}
  
  \CommentTok{#qqnorm(residuals(md_3))}
  
  \KeywordTok{anova}\NormalTok{(md_}\DecValTok{2}\NormalTok{, md_}\DecValTok{3}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{model_dt}\OperatorTok{$}\NormalTok{dates <-}\StringTok{  }\KeywordTok{as.integer}\NormalTok{(model_dt}\OperatorTok{$}\NormalTok{dates)}
  
  \NormalTok{md_}\DecValTok{4}\NormalTok{ <-}\StringTok{ }\KeywordTok{gamm4}\NormalTok{(turnout }\OperatorTok{~}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{types }\OperatorTok{+}
  \StringTok{               }\NormalTok{pct_urban }\OperatorTok{+}\StringTok{ }\NormalTok{pct_white }\OperatorTok{+}\StringTok{ }\NormalTok{pct_vbm}\OperatorTok{*}\NormalTok{types }\OperatorTok{+}\StringTok{ }\KeywordTok{s}\NormalTok{(dates, }\DataTypeTok{k =} \DecValTok{7}\NormalTok{), }
               \DataTypeTok{random =}\OperatorTok{~}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{county), }
               \DataTypeTok{data =}\NormalTok{ model_dt)}
  
  \KeywordTok{summary}\NormalTok{(md_}\DecValTok{4}\OperatorTok{$}\NormalTok{mer)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#plot(fitted(md_4$mer), residuals(md_4$mer))}
  
  \CommentTok{#qqnorm(residuals(md_4$mer))}
  \end{Highlighting}
  \end{Shaded}
  ...
  step by step and separatelly for each group of variables (grouping by
  ...
  inlcudes all different variables.
  ...
  the assumption of independance--these counties are, after all, in the
  ...
  deviation and \(\gamma_0\), an intercept coefficient. Let's say now that
  along with the one ballot, I was given a short list of
  ...
  before, I do not assume independance, the model can be written as:
  ...
  individual level, thus making it relativelly easy to build further
  ...
  individual-level data for gender, so \(n^{\text{indiv vars}} = 1\).
  ...
  would include four distinct indicators; one for each election type.
  ...
  Unaffiliated. Age is tricky; for now the options would be: straight up
  inclusion as an integer, inclusion as a cubic polynomial, unclusion as a
  2nd degree polynomial, inclusion in some form of spline function.
  
  \subsection{Estimation with two types of
  data}\label{estimation-with-two-types-of-data}
  
  After the work of setting up the four models at four different levels of
  observation, combining them in twos should be failry straightforward. To
  avoid being nedlessly cumulative, I will pursue this combination for
  ...
  You will notice that I have ommited the subscript for all beta
