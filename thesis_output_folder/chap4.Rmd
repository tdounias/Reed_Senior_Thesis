...
```{r test table, results = "asis", echo = FALSE, message=FALSE}
source("~/Desktop/Reed_Senior_Thesis/tables/county_coef_table.R")
pandoc.table(table_data, caption = "Estimated county level coefficients \\label{tab:county_coef}")
```
...
Assume that the ballot I am trying to assess completion for has the name of the county it is from written on it. There are two ways I can think of for predicting $\mathbb{P(y_i = 1)}$. First, assume that each different county has a different, independent $\mathbb{P(y_i = 1)}$, then:
...
Where $w_{l[i], i'}$ is the l-th value of the i'-th variable. For the time being I have two different variables that describe individual elections: date and type. I choose to fit a glm with some smoothing spline function for year. This would also include four distinct indicators for election type. I name this final model (including a smoothing spline for year) **Model 4**. Model 4 would not be a mixed effects model, since all the variability between elections is incorporated in election type and election year--with those two variables I can fully describe each election.
